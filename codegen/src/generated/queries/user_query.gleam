import generated/models/user.{
  type User, type UserField, Bio, CreatedAt, Email, Id, Name, UpdatedAt, User,
}
...
pub fn find_by_field(
  ...
  let sql =
    "SELECT id, name, email, bio, created_at, updated_at FROM users WHERE "
    <> field_name
    <> " = $1"
...
pub fn create(
  ...
  let sql =
    "INSERT INTO users (id, name, email, bio) VALUES ($1, $2, $3, $4) RETURNING id, name, email, bio, created_at, updated_at"
...
pub fn update(
  ...
  let sql =
    "UPDATE users SET name = $2, email = $3, bio = $4 WHERE id = $1 RETURNING id, name, email, bio, created_at, updated_at"
  ...
  case updates {
    ...
    _ -> {
      ...
      let sql =
        "UPDATE users SET "
        <> set_clauses
        <> " WHERE id = $1 RETURNING id, name, email, bio, created_at, updated_at"
